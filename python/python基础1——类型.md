...
date: 2022-6-28 10:59:20

updated: 2022-6-28 10:59:20
...
+ 列表推导式
  ...
  + 简单版`[表达式 for x in 可迭代对象]`
  
  + 带条件版`[表达式 for x in 可迭代对象 if 条件语句]`
  
  + 带条件带else版 `[表达式 if 条件语句 else 条件语句 for x in 可迭代对象]`
...
+ 集合是一个无序的，不重复元素序列，用大括号或`set()`函数创建

+ 基本操作
  
  + 判断元素是否在集合内`a in seta`
  
  + 集合运算，可以进行交并补等运算
  
  + 添加元素`seta.add(a)`
  
  + 移除元素`seta.remove(a)`或`seta.discard(a)`

```python
>>> basket = {"1","1","2","3"}
>>> print(basket)
{'1', '3', '2'}
>>> basket - casket   # 集合basket中包含而集合casket中不包含的元素
{'1'}
>>> basket | casket  # 集合basket或casket中包含的所有元素
{'6', '5', '2', '1', '3', '7'}
>>> basket & casket  # 集合basket和casket中都包含了的元素
{'3', '2'}
>>> basket ^ casket  # 不同时包含于basket和casket的元素
{'7', '1', '6', '5'}
```
